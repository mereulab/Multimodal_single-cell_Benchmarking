...
if __name__ == '__main__':
    ...
    data.obs['Deepscore_HCA_l1_Clean'] = metadata['Deepscore_HCA_l1_Clean']

    clean_obs = data.obs.dropna(subset=['Deepscore_HCA_l1_Clean'])
    data = data[data.obs.index.isin(clean_obs.index)]
    
    data.obs['Deepscore_HCA_l1_Clean'] = [i.replace('/','_') for i in data.obs['Deepscore_HCA_l1_Clean']]
    data.obs['Deepscore_HCA_l1_Clean'] = data.obs['Deepscore_HCA_l1_Clean'].astype('category')
    

    #data = data[:,data.var['selected']].copy()
    print(data.obs.columns)
    #data.obs['test'] = 'test'
    snap.tl.macs3(data, groupby='Deepscore_HCA_l1_Clean', qvalue=0.05)
    data.write('objects/MACS3_l1.h5ad', compression='gzip')
    ...
    import pickle
    with open('macs_peak_l1.pickle', 'wb') as handle:
        pickle.dump(peak_mat, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open('merged_peaks_l1.pickle', 'wb') as handle:
