...
if __name__ == "__main__":
   ...
   # tqdm.disable = True  # Disable all tqdm progress bars
    ...
    print(adata)
    print(peaks_selected)
    print('Cell type', group1_, 'Num PEAKS ', np.sum(peaks_selected),'Peaks in adata', np.sum(adata.var.index.isin(peaks.index[peaks_selected])), '\tNum cells', np.sum(bar_group_1))
    ...
    diff_peaks.to_csv(f'csv/l1/DAR_{group1}.csv', index_label='Feature_name')
