...
if __name__ == "__main__":
    ...
    logging.getLogger('snapatac2').setLevel(logging.ERROR)


    group2 = 'PT'  # Get the parameter from the command line
    group1 = 'DCT'



    with open("../macs_peak_l1.pickle", "rb") as file:
        adata = pickle.load(file)

    with open("../mergued_peaks_l1.pickle", "rb") as file:
        peaks = pickle.load(file)

    marker_peaks = snap.tl.marker_regions(adata, groupby='Deepscore_HCA_l1_Clean', pvalue=0.01)
    ...
    bar_group_1 = adata.obs['cell_type'] == group1
    bar_group_2 = adata.obs['cell_type'] == group2
    
    print(peaks)
    print(bar_group_1)
    
    peaks_selected_1 = peaks[group1].to_numpy()
    peaks_selected_2 = peaks[group2].to_numpy()
    peaks_selected = peaks_selected_1 | peaks_selected_2
    ...
    diff_peaks.to_csv(f'csv/Comparison_DCT_PT.csv', index_label='Feature_name')
    
    # Example usage:
    csv_file = f'csv/Comparison_DCT_PT.csv'
    bed_file = f'csv/Comparison_DCT_PT.bed'
    csv_to_bed(csv_file, bed_file)
    
    
    group1 = 'PT'  # Get the parameter from the command line
    group2 = 'DCT'
    ...
    diff_peaks.to_csv(f'csv/Comparison_PT_DCT.csv', index_label='Feature_name')
    
    # Example usage:
    csv_file = f'csv/Comparison_PT_DCT.csv'
    bed_file = f'csv/Comparison_PT_DCT.bed'
    csv_to_bed(csv_file, bed_file)
